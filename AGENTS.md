...
- Use atomic writes (`tmp.write_text` -> `os.replace`) and schema validation in `convert/convert.py`
- Follow commit discipline and logging rules from handover
- **Standard Source Map**: Any extracted data must be validated against `convert/schema.json` and wrapped in the `meta` + `entries` envelope as defined in the UPSTREAM_MAP_DESIGN.
